...
def complete(prompt):
    ...
    try:
        if active_completion_engine == "gpt-j":
            ...
            response = (post("http://api.vicgalle.net:5000/generate", params=payload)).json()
            result = response["text"]
        if active_completion_engine == "j1-large":
            result = complete_ai21("j1-large")
        if active_completion_engine == "j1-jumbo":
            result = complete_ai21("j1-jumbo")
...
def on_message(message: str):
    ...
    if message.startswith("!reset"):
        context = []
        print(options.reset_message)
        return

    if message.startswith("!status"):
        print("{} Status".format(options.name))

        print("Completion engine in use: {}"
              .format(engine_info[active_completion_engine]["text"]))

        current_usage = token_usage[active_completion_engine]
        max_usage = engine_info[active_completion_engine]["maxTokens"]
        print("Token usage: {0}/{1} ({2})"
              .format(current_usage, max_usage, calculate_token_percentage_used(active_completion_engine)))

        now = time.time()
        print("Last request successful? {}"
              .format("No requests made since startup" if last_time == 0 else ("Yes" if last_successful else "No")))

        print("Time since last request: {}"
              .format("No requests made since startup" if last_time == 0 else str(time_since_last) + " seconds"))
    ...
    author_identifier = "[" + options.author_name + "]"
    context.append(author_identifier + " " + message[0:options.message_cutoff])
